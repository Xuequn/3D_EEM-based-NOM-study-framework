"""
CNN model for predicting values from eem labels.
...
# Load preprocessed EEM data and labels
X = np.load("./eem_features.npy")  # Shape should be (n_samples, 81, 81, 1)
y = np.load("./eem_labels.npy")  # Shape should be (n_samples,)
...
for i in range(10):
    ...
    train_max = np.max(X_train)   # Use only training set maximum
    X_train = X_train.astype('float32') / train_max
    X_val = X_val.astype('float32') / train_max
    X_test = X_test.astype('float32') / train_max
    dump({'feature_max': train_max}, f'./scalers_feature_{random_state}.joblib')
    ...
    input_layer = tf.keras.Input(shape=(81, 81, 1))
    ...
    y_pred_test_original = scaler_labels.inverse_transform(y_pred)
    ...
    with pd.ExcelWriter(f'predictions_{random_state}.xlsx') as writer:
    ...
    # Save complete model
    model.save(f'unplotted_RCPM_{random_state}.h5')
